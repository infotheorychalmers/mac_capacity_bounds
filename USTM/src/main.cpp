...
for( int iter = 0;iter<SNRCount;iter++)
{
	...
	MutualInformation  mutInfoObj(N,K,T,itpp::inv_dB(startSNR+iter*deltaSNR),MCX,MCY,resultHolder(iter));
...
std::stringstream ss;
ss<<"InfoValsUSTM_K"<<K<<"_N"<<N<<"_T"<<T<<"_"<<boost::posix_time::to_iso_string(now)<<".it";
itpp::it_file outFile;
outFile.open(ss.str().c_str());
outFile<<itpp::Name("InfoValsUSTM")<<resultHolder;
